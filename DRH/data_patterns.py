...
df = df[["related_q", "related_q_id", "answers", "answer_val", "related_parent_q", "entry_name", "entry_id", "end_year"]]
...
q_total = len(df["related_q_id"].drop_duplicates())
q_binary = len(df_b["related_q_id"].drop_duplicates()) 
print(f"total Q: {q_total}\nbinary Q: {q_binary} ({round(q_binary/q_total*100,2)})%")

# create nodes X samples X NA
## fill unanswered questions with NaN
df_nan = fill_grid(df_b, "related_q_id", "entry_id", "NaN")
n_samples = len(df_nan["entry_id"].drop_duplicates())
n_nodes = len(df_nan["related_q_id"].drop_duplicates())
...
d_lineplot = calc_nodes_samples(
    ...
    c_ent_id = "entry_id",
    c_ans = "answer_types",
    s_minval = "NaN", # should be NAN and DK
