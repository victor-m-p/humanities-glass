...
class RBM(nn.Module):
   def __init__(self,
               n_vis=784, # number visibile
               n_hin=500, # number hidden
               k=5): # number of repetitions
        super(RBM, self).__init__()
        self.W = nn.Parameter(torch.randn(n_hin,n_vis)*1e-2) # tensor from normal dist
        self.v_bias = nn.Parameter(torch.zeros(n_vis)) # tensor w. zeros
        self.h_bias = nn.Parameter(torch.zeros(n_hin)) # tensor w. zeros
        self.k = k # number of repetitions
    
   def sample_from_p(self,p):
       # F.relu(): rectified linear element-wise
       # torch.rand(): tensor with random numbers (0, 1)
       # torch.sign(): tensor with sign of elements (1, -1, 0).
       # Variable(): deprecated (perhaps we can just remove). 
       # not sure what p is. 
