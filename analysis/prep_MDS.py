...
for n_top_states in [10, 20]: 
    ...
    for i, values in enumerate(zip(p_ind, p_val, top_states)): 
        ...
        lst_neighbors = []
        for idx_datastate, val_datastate in enumerate(allstates):
            h_dist = np.count_nonzero(val_top!=val_datastate)
            if h_dist <= n_nearest and idx_top != idx_datastate: 
                p_datastate = p[idx_datastate]
                lst_neighbors.append((i, idx_top, idx_datastate, p_top, p_datastate, h_dist))
            else: 
                pass
        df_neighbor = pd.DataFrame(
            lst_neighbors, 
            columns = ['node_id', 'p_ind_focal', 'p_ind_other', 'p_raw_focal', 'p_raw_other', 'hamming_neighbor'])
