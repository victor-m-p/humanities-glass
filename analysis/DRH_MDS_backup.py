...
'''
This should eventually get its own document
'''
## data states not in top 500 configurations ##
## hamming distance

# load probabilities for all configurations that we have 
d = pd.read_csv('../data/analysis/d_likelihood_nrow_660_ncol_21_nuniq_20_suniq_581_maxna_10_NN1_LAMBDA0_453839.csv')
mat = np.loadtxt('../data/analysis/mat_likelihood_nrow_660_ncol_21_nuniq_20_suniq_581_maxna_10_NN1_LAMBDA0_453839.txt')

## maximum likelihood configurations
d_max = d.groupby('entry_id')['p_norm'].max().reset_index(name='p_norm')
d_max = d.merge(d_max, on = ['entry_id', 'p_norm'], how = 'inner')
#d_max['p_ind'] = [x-1 for x in d_max['p_ind']] # difference in indexing (fix)
...
## distance from non top500 to top500
#same = h_distances[:l_top500, :l_top500] # same as what we used (distances)
new = h_distances[:l_top500, l_top500:]
avg_deviation = np.sum(new, axis = 0)/l_datastates
min_deviation = np.amin(new, axis = 0)
d_hamming = pd.DataFrame({
    'h_avg': avg_deviation, 
    'h_min': min_deviation
}
)

d_hamming['index'] = d_hamming.index
d_max['index'] = d_max.index
d_dist = d_hamming.merge(d_max, on = 'index', how = 'inner')

# actually, more states COULD be in the data...

in_both = node_attr[node_attr['datastate'] == 'Yes']
in_both[['node_index', 'entry_id', 'entry_name', 'p_ind', 'size']]
d_dist.merge(in_both, on = ['p_ind', 'entry_id'], how = 'left', indicator = True)

# do we know which ones are datastates? 
substates_id
