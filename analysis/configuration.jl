...
module cn
using Printf, Statistics, Random, Distributions, DelimitedFiles, CSV, DataFrames, IterTools, StatsBase, Chain, FStrings
...
mutable struct Configuration 
    ...
    function Configuration(id, configurations, configuration_probabilities)
        ...
        # does this have to be down here?
        ...
        self.targets = collect(1:self.len)
        ...
        self.new_move = function(fixed_idx="false", n=1)
            perm = randperm(length(self.targets))
            proposed_flip = self.targets[perm[1:n]]
            proposed_flip = proposed_flip[proposed_flip .!= fixed_idx]
            # if no flip occurs, return self
            if proposed_flip == [] return self end
            flipped_conf = self.flip_at_indices(proposed_flip)
            ...
            # if move binary then move to flipped 
